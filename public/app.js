...
var Country = function(id) {
    ...
    if (this.continent) {
        if (!window.continentIndex[this.continent]) {
            window.continentIndex[this.continent] = new Continent(this.continent, []);
            window.allContinents.push(window.continentIndex[this.continent]);
        }
        window.continentIndex[this.continent].countries.push(this);
    }
...
Country.prototype.generatePriority = function() {
    ...
    var median = 110860;
    var sizeRank = sigmoid(param, median, this.area) + 0.5;
    return (Math.pow(this.val, 5)/sizeRank) * 500;
};

Country.prototype.generatePreferences = function(result) {
    var cx = this.centroid.x;
    var cy = this.centroid.y;
    var scaleFactor = this.val/0.5; // 0.5 = 'median' of logistic function data
    for (var i = 0; i < this.points.length; i++) {
        var p = this.points[i];
        var endX = cx + (p.destX - cx) * scaleFactor * 1;
        var endY = cy + (p.destY - cy) * scaleFactor * 1;
...
Continent.prototype.setValue = function(metricData) {
    this.val = 0.0;
    var countries = this.countries;
    ...
    for (var i = 0; i < countries.length; i++) {
        ...
        console.log(country.id);
        console.log("metricData[country.id]");
        console.log(metricData[country.id]);
        country.val = metricData[country.id];
        this.val += country.val;
    }
    console.log("OMG VALUE IS SO LIKE LIKE LIKE " + this.val);
    this.val /= n;
};
...
function deformContinents(metricData) {
    var NA = window.continentIndex['North America'];
    var SA = window.continentIndex['South America'];
    NA.deform(metricData);
    SA.deform(metricData);
}
...
function initCanvas() {
  ...
  window.ctx.clearRect(0, 0, 1080, 540);

  window.ctx.beginPath();
  window.ctx.moveTo(0, 0);
  window.ctx.moveTo(0, 540);
  window.ctx.lineTo(1080, 540);
  window.ctx.moveTo(1080, 0);
...
function beginEventLoop() {
  window.setInterval(function() {
    window.ctx.clearRect(0, 0, 1080, 540); // Clearing the screen
...
function mouseMove() {
  $('#map').mousemove(function(e) {
    ...
    if (country != null) {
      ...
      $('#info-display').html(country.name + '<br>' + country.continent);
    } else {
      $('#info-display').html('');
    }
  });
